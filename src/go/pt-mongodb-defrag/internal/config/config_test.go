...
func TestPreventAutoMergeValidateSuccess(t *testing.T) {
	cfg := Config{
	...
	if cfg.Database != "db" || cfg.Collection != "coll" {
		t.Fatalf("expected Database=db Collection=coll, got %s.%s", cfg.Database, cfg.Collection)
	}
}

func TestPreventAutoMergeValidateMissingNamespace(t *testing.T) {
	cfg := Config{
		PreventAutoMerge: true,
		ChunkID:          "chunk_1",
		ChunkQuery:       "sk=15000",
	...
	if err := cfg.Validate(); err == nil {
		t.Fatalf("expected validation error for missing -namespace")
	}
}

func TestPreventAutoMergeValidateMissingChunkID(t *testing.T) {
	cfg := Config{
		PreventAutoMerge: true,
		Namespace:        "db.coll",
		ChunkQuery:       "sk=15000",
	...
	if err := cfg.Validate(); err == nil {
		t.Fatalf("expected validation error for missing -chunk-id")
	}
}

func TestPreventAutoMergeValidateMissingChunkQuery(t *testing.T) {
	cfg := Config{
		PreventAutoMerge: true,
		Namespace:        "db.coll",
		ChunkID:          "chunk_1",
	...
	if err := cfg.Validate(); err == nil {
		t.Fatalf("expected validation error for missing -chunk-query")
	}
}

func TestPreventAutoMergeValidateMissingTargetShard(t *testing.T) {
	cfg := Config{
		PreventAutoMerge: true,
		Namespace:        "db.coll",
		ChunkID:          "chunk_1",
	...
	if err := cfg.Validate(); err == nil {
		t.Fatalf("expected validation error for missing -target-shard")
	}
}
